...
[lyrics.js](src/lyrics.js)가 가사를 가져와 파싱·캐싱하고, [lyricsSession.js](src/lyricsSession.js)가 **가사판 하나의 수명**을 관리합니다. `/가사`는 세션을 켜고 끄기만 합니다. 소스는 lrclib.net이며 JSON을 주므로 파싱 의존성이 없습니다.

디스코드에는 애니메이션이 없습니다. **가사가 움직이는 것은 메시지 하나를 계속 편집하는 것**이고, 5줄 창의 가운데를 현재 줄로 고정해 스크롤을 흉내냅니다.

**대비는 글씨 크기가 아니라 색으로 줍니다.** embed description 안에 ` ```ansi ` 코드블록을 넣고 SGR 코드로 지난 줄은 회색, 현재 줄은 굵은 노랑, 다음 줄은 흰색으로 칠합니다. 헤딩(`## `)과 서브텍스트(`-# `)를 섞는 방식을 처음에 썼다가 버렸는데, **한 블록 안에 글씨 크기가 세 종류가 되어 줄 높이가 들쭉날쭉해 보이기 때문입니다.** 코드블록은 모든 줄이 같은 크기라 화면이 흔들리지 않습니다. `LYRICS_PLAIN_STYLE=1`이면 색 없이 화살표(`▸`)로 그립니다 — ANSI가 깨져 보이는 환경을 위한 탈출구입니다.

**`sanitizeLyricLine`을 거치지 않은 가사를 창에 넣지 마세요.** 백틱이 하나라도 남으면 **가사 한 줄이 코드블록을 통째로 닫아** 뒤의 모든 줄이 평문으로 쏟아지고, 제어문자(ESC)가 남으면 가사가 제 색을 지정해 화면이 엉킵니다. 64자를 넘는 줄을 자르는 것도 같은 함수인데, 코드블록에서 줄이 접히면 색이 이어진 줄까지 번집니다. `renderWindow`는 곡의 처음·끝에서 빈 줄로 창의 높이를 채웁니다.

**`LyricsSession`에 `player`를 넘기지 마세요.** 지금은 위치를 읽는 클로저(`getPositionMs`) 하나만 받고, `stop()`이 그것을 `null`로 만듭니다. 세션이 어딘가에 남아도 `GuildMusicPlayer`(대기열·음성 연결·ffmpeg)가 GC되는 이유가 이것입니다. player를 넘기도록 고치면 타이머 콜백이 플레이어를 통째로 붙잡아 **곡마다 수 MB가 샙니다.** `test/lyricsSession.test.js`가 `stop()` 후 참조가 끊겼는지 검사합니다.

**`Message` 객체를 보관하지 않습니다.** `index.js`가 `MessageManager`를 0으로 막아 두었으므로 ID 문자열만 들고 `channel.messages.edit(id, ...)`로 REST를 칩니다. `interaction.editReply`는 **토큰이 15분이면 만료**되어 긴 곡에 쓸 수 없습니다.

**타이머는 `setInterval`이 아니라 자기 재예약 `setTimeout`입니다.** 줄이 바뀌는 시점에만 깨어나고, 최소 간격(`LYRICS_EDIT_INTERVAL_MS`, 기본 3초)이 걸려 있습니다. 디스코드의 편집 레이트리밋에 걸리면 discord.js가 요청을 큐에 쌓아 **가사가 음악보다 점점 뒤처집니다.** 그 사이 지나간 줄은 따라가지 않고 다음 편집 때 현재 위치로 한 번에 건너뜁니다 — 밀림을 누적시키지 않는 것이 요점입니다.

정리 지점이 **네 곳**입니다: `_play` 진입, `destroy()`, `join()`의 채널 이동, `/가사 끄기`. 종료 경로를 추가할 때 `_stopLyrics()`를 빠뜨리면 아무도 없는 채널에서 계속 편집됩니다.

가사판이 밀려 올라가도 찾을 수 있게 **고정(pin)** 합니다. 인텐트가 `Guilds`/`GuildVoiceStates` 둘뿐이라 `messageCreate`를 받을 수 없어 "밀리면 다시 보내기"를 구현할 수 없기 때문입니다. `GuildMessages`는 특권 인텐트가 아니지만, 모든 메시지마다 이벤트가 깨어나는 값을 512MB 인스턴스에서 치를 이유가 없어 추가하지 않았습니다.

기본 표시 위치는 **봇이 들어가 있는 음성 채널의 텍스트 채팅**입니다(v14의 음성 채널은 텍스트 채널이기도 합니다). 음성 채널 ID는 `connection.joinConfig.channelId`에 이미 있어 새로 붙잡는 참조가 없습니다. 쓸 권한이 없으면 `isSendable()`로 걸러 조용히 텍스트 채널로 떨어집니다.

캐시 상한이 세 겹입니다: 항목 수(`LYRICS_CACHE_SIZE`, 기본 50 — 삽입 순서를 쓰는 `Map`이라 이것만으로 LRU), 곡당 32KB, 수명 6시간. **최악의 경우 1.6MB로 고정됩니다.** `melon.js`처럼 결과가 비면 캐시를 덮어쓰지 않고, **못 찾은 결과(`null`)도 캐싱합니다** — 안 그러면 한 곡 반복에서 같은 조회를 끝없이 반복합니다.

**가사 설정은 DB에 없습니다.** 프로세스 메모리에만 있어 재시작하면 꺼진 상태로 돌아갑니다. 칼럼 추가는 `ensureColumn` + 양쪽 백엔드 + 마이그레이션 테스트까지 따라오므로, 기능이 쓸 만한지 확인한 뒤에 붙입니다. `DISABLE_LYRICS=1`로 통째로 끌 수 있고 동시 표시는 `LYRICS_MAX_SESSIONS`(기본 3)으로 제한합니다.
